...
import { Category, Pet, PetCreateParams, PetFindByStatusParams, PetFindByStatusResponse, PetFindByTagsParams, PetFindByTagsResponse, PetUpdateByIDParams, PetUpdateParams, PetUploadImageParams, PetUploadImageResponse, Pets } from './resources/pets';
import { User, UserCreateParams, UserCreateWithListParams, UserLoginParams, UserLoginResponse, UserUpdateParams, Users } from './resources/users';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['YOLKEN_TEST6_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['YOLKEN_TEST6_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Yolken Test6 API. 
 */
export class YolkenTest6 {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['YOLKEN_TEST6_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('YOLKEN_TEST6_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.YolkenTest6Error(
        'The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the YolkenTest6 client with an apiKey option, like new YolkenTest6({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    this.timeout = options.timeout ?? YolkenTest6.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('YOLKEN_TEST6_LOG'), 'process.env[\'YOLKEN_TEST6_LOG\']', this) ?? defaultLogLevel;
  ...
  static YolkenTest6 = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static YolkenTest6Error = Errors.YolkenTest6Error;
...
YolkenTest6.Pets = Pets;
YolkenTest6.Store = Store;
YolkenTest6.Users = Users;

export declare namespace YolkenTest6 {
