...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { PETSTORE_API_KEY: requireValue(readEnv('PETSTORE_API_KEY') ?? client.apiKey, 'set PETSTORE_API_KEY environment variable or provide apiKey client option'), YOLKEN_TEST6_BASE_URL: readEnv('YOLKEN_TEST6_BASE_URL') ?? client.baseURL ?? undefined };
