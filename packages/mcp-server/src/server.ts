...
export { ClientOptions } from 'yolken-test6';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/yolken-test6',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the yolken-test6 MCP server. You will use Code Mode to help the user perform
